...
def tri_training(labeled_path,
    ...
    for i in range(3):
        ...
        test_prediction_paths.append(add_suffix_to_path(prediction_path, suffix=i, split_by='.'))
    ...
    while True:
        ...
        os.makedirs(os.path.dirname(test_prediction_paths[0].format(iteration)), exist_ok=True)
        ...
        for i in range(3):
            modify_config_file(DEFAULT_PREDICT_CONFIG_PATH,
                               TEMP_PREDICT_CONFIG_PATH,
                               {
                                   ...
                                   'predictions_path': test_prediction_paths[i].format(iteration)
                               })
            script = PREDICT_SCRIPT.format(config_path=TEMP_PREDICT_CONFIG_PATH)
            logger.info(f'Round #{iteration}: Predict on test data on model {i}')
            subprocess.run(script, shell=True, check=True)
        evaluate_tri_training(DEFAULT_TEST_PATH,
                              test_prediction_paths[0].format(iteration),
                              test_prediction_paths[1].format(iteration),
                              test_prediction_paths[2].format(iteration),
                              logger)
        iteration += 1
